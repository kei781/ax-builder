...
동작:
1. 컨테이너 존재 확인
2. `docker restart <container_id>` — 빠르고 가벼움. 컨테이너 재생성 아님.
3. 헬스체크 10초 폴링 (HTTP HEAD)
4. 통과 → WS `completion(state=deployed)` / 실패 → WS `error(kind=restart_failed)` + 상태 유지(`deployed`)
