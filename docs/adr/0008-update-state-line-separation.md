...
3. Building Agent 성공 후 env-deploy가 **새 포트**에 **새 컨테이너**를 띄우고 헬스체크:
    - **헬스체크 성공** → 이전 컨테이너 제거, 새 컨테이너 canonical 이름으로 swap-rename, `current_version += 1`, `previous_*` clear, `deployed` 전이.
...
**D4-ter 컨테이너 명명 swap (2026-04-24 추가 — 회고 §10)**: 위 3번 "새 컨테이너를 띄우고" 단계가 Docker daemon의 컨테이너 이름 unique 제약과 충돌해 v2 update가 영구 bounce되는 사고가 발생. 원본 §D4는 "두 컨테이너 동시 보유" 모델만 명시하고 명명 규칙은 ADR 스코프 밖이었지만, `DockerService.createContainer`가 canonical 단일 이름(`project-{projectId}`)만 사용해 update 사이클의 새 컨테이너가 옛 컨테이너와 같은 이름으로 충돌(HTTP 409) → 새 컨테이너 자체를 못 띄움.

대응:
- `DockerService.canonicalName(projectId)` → `project-{projectId}` 분리.
- `createContainer(..., nameSuffix?)` — update 사이클에서 호출 시 `update-{Date.now()}` suffix 부여해 임시 이름으로 생성.
- 헬스체크 통과 직후 옛 컨테이너 제거 + 새 컨테이너를 `renameContainer(id, canonicalName)`로 swap. 두 단계는 같은 try에 묶어 부분 실패 시 일관성 확보. rename 실패는 치명적이지 않음(컨테이너는 임시 이름으로 살아있고 DB의 container_id가 정확히 가리킴 — 다음 사이클이 또 임시 suffix 쓰면 충돌 없음).

이로써 "두 컨테이너 동시 보유" 모델이 Docker primitive 위에서 비로소 실효. 회고 §10 교훈: ADR 작성 시 구현 primitive 제약을 spike하는 단계 권장.
