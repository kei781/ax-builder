...
**세분 복원 (회고 §8, 2026-04-24)**: 업데이트 사이클 안에서 AI가 `write_prd`를 여러 번 호출할 수 있고, 각 호출마다 직전 PRD가 `PRD.md.bak.{timestamp}`로 자동 스냅샷된다. UPDATE 모드에선 도구 레벨 가드(변경률 < 30% 또는 H1 변경 + 유사도 < 70% reject)도 함께 걸려 있어 "간식 당번 → Todo App" 같은 도메인 전체 교체 시도는 AI 턴 중에 차단된다. 과거 사고로 이미 어긋난 프로젝트는 Chat 페이지 상단 호박색 **"프로젝트 제목과 PRD 제목이 달라요"** 배너 + **[이전 PRD 버전으로 되돌리기]** 모달로 복원 가능(owner 한정). API는 `GET /api/projects/:id/prd/backups` / `POST /api/projects/:id/prd/restore`.

### 유저 페르소나 런타임 주입 (회고 §8, 2026-04-23)

`users.profile_is_developer` / `profile_explain_depth` 값이 planning-agent 시스템 프롬프트 최상단에 매 턴 주입된다. 비개발자 유저에겐 단위/통합/E2E 같은 용어 사전이 금지되고 "테스트" → "버튼 눌러서 동작 확인"으로 번역하는 규칙이 강제. 프로젝트 제목도 프롬프트에 고정돼 "할 일 관리 앱"류 일반화 환각을 차단. UPDATE 모드에선 기존 PRD가 `<EXISTING_PRD>` 블록으로 선주입돼 AI가 "못 읽었다"고 주장하지 못하게 구조화.
