...
- 프로젝트 디렉토리에 `CLAUDE.md`와 `.claude/skills/prd-sync`가 이미 있습니다.
  claude CLI가 자동 로드하므로 PRD/DESIGN 준수와 동기화는 기본 행동입니다.
- 서브에이전트 프롬프트 끝에 반드시 **"PRD.md/DESIGN.md 동기화하고 마무리"**를 명시.
...
```bash
cd {project_path} && claude -p "CLAUDE.md, PRD.md, DESIGN.md를 정독하고 프로젝트 스캐폴드를 만드세요.
규칙:
...
- DESIGN.md의 컬러/폰트를 CSS 변수(--color-primary 등)로 정의한 public/styles.css 생성
- 필요한 npm 패키지까지 설치 완료
- 스캐폴딩 과정에서 PRD/DESIGN의 [미정] 항목을 구체화했다면 해당 섹션을 Edit으로 실제 값으로 갱신
완료 후 'SCAFFOLD_DONE'을 출력하세요." --allowedTools "Bash Read Write Edit" --permission-mode bypassPermissions --no-session-persistence
```
...
```bash
# Backend 에이전트 (백그라운드)
cd {project_path} && claude -p "CLAUDE.md, PRD.md를 정독하고 백엔드 API를 구현하세요.
...
프론트엔드 쪽은 건드리지 말고 API만 완성.
**구현하면서 PRD.md 기술 설계 섹션에 실제 API 엔드포인트/스키마를 Edit으로 기록**하세요 (예: GET /api/items → 반환 필드).
완료 후 'BACKEND_DONE'을 출력하세요." --allowedTools "Bash Read Write Edit" --permission-mode bypassPermissions --no-session-persistence > /tmp/backend_{port}.log 2>&1 &
BACKEND_PID=$!

# Frontend 에이전트 (백그라운드)
cd {project_path} && claude -p "CLAUDE.md, PRD.md, DESIGN.md를 정독하고 프론트엔드 UI를 구현하세요.
- public/index.html을 메인 엔트리로
- DESIGN.md의 컬러/타이포/컴포넌트 규칙을 **엄격히** 준수 (hex 그대로, 폰트 패밀리/크기 준수)
- PRD의 모든 화면/페이지를 구현 (SPA 또는 multi-page)
- API 호출은 fetch()로 /api/* 엔드포인트 사용 (Backend 에이전트와 계약)
- 반응형 레이아웃 (모바일 대응)
- 시각적으로 완성도 높게 — 디자인 목업 수준
백엔드 코드(src/)는 건드리지 말고 public/만 작업.
**구현 중 DESIGN.md의 컴포넌트/레이아웃 섹션에 실제 사용된 클래스/토큰을 Edit으로 보강**하세요.
완료 후 'FRONTEND_DONE'을 출력하세요." --allowedTools "Bash Read Write Edit" --permission-mode bypassPermissions --no-session-persistence > /tmp/frontend_{port}.log 2>&1 &
FRONTEND_PID=$!
...
완료 후 'INTEGRATION_DONE' 또는 'INTEGRATION_FAILED: <이유>'를 출력하세요." --allowedTools "Bash Read Write Edit" --permission-mode bypassPermissions --no-session-persistence
```
...
}}" --allowedTools "Bash Read" --permission-mode bypassPermissions --no-session-persistence
```

### PHASE 6: Fix (QA 실패 시)
QA가 failed를 냈으면, Fixer 에이전트 호출:
```bash
cd {project_path} && claude -p "CLAUDE.md 규칙 준수. QA에서 다음 이슈가 발견됐습니다:
[여기에 features_failed JSON 붙여넣기]
각 이슈를 수정하세요. PRD.md/DESIGN.md 준수.
**수정 결과가 PRD/DESIGN과 달라졌다면 두 파일도 Edit으로 동기화**하세요.
완료 후 'FIX_DONE'을 출력하세요." --allowedTools "Bash Read Write Edit" --permission-mode bypassPermissions --no-session-persistence
```
