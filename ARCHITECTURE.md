...
- `BuildingRunner.onModuleInit()`이 startup 시 `building` / `qa` / `updating` / `update_qa` 상태 프로젝트를 전수 검사 (새로 시작한 순간 `processes` Map은 비어있으므로 고아 확정).
- 정리: build `cancelled` + 진행 phase `cancelled` + project → `failed`. bounce 메시지에 "orchestrator 재시작으로 빌드 프로세스가 고아가 됐어요. '다시 빌드'로 재시도하세요."
