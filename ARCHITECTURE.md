...
| 도구 | 허용 |
|---|---|
| `write_prd` | ✅ (UPDATE 모드 가드 + 자동 백업 — 아래) |
| `write_design` | ✅ |
| `ask_user` | ✅ |
| `search_memory` | ✅ |
| `update_memory` | ✅ |
...
도구 핸들러는 `(ToolCtx, args)` 시그니처를 받는다(`planning-agent/app/agent/tools/base.py`). `ToolCtx = (project_id, session_id, is_update_mode)`로, `registry.dispatch(...)`가 loop의 턴 컨텍스트를 주입한다. 핸들러가 첫 빌드/업데이트 라인을 분기하고 세션 단위로 상태를 추적할 수 있다.

**`write_prd` 가드 (회고 §8.7~9, 2026-04-24)**:
- UPDATE 모드 한정. 기존 PRD와 `difflib.SequenceMatcher` ratio `< 0.3` 또는 H1이 바뀌고 ratio `< 0.7`이면 핸들러가 reject하고 AI에게 구체 가이드 반환.
- 첫 빌드 / UPDATE 무관, 매 호출마다 기존 PRD가 있으면 `PRD.md.bak.{iso8601Z}`로 자동 스냅샷. 유저는 orchestrator `GET /prd/backups` + `POST /prd/restore`로 복원.
- 원 설계에서 "UPDATE 모드엔 `Read` 도구로 먼저 읽어라"고 지시했지만 Read 도구 자체가 registry에 없었다(실패). 대체로 `loop._build_initial_messages`가 UPDATE 모드에 PRD.md를 읽어 system prompt 말미에 `<EXISTING_PRD>` 블록으로 선주입한다 — AI가 "못 읽었다"고 주장할 수 없는 구조.
...
### 3.6 시스템 프롬프트 런타임 조립 (회고 §8, 2026-04-23~24)

`build_system_prompt()`는 고정 상수가 아니라 턴마다 3축으로 조립한다:
1. **유저 페르소나 preamble** — orchestrator `chat.service.sendUserMessage`가 `projectRepo.findOne({relations:['owner']})`로 owner를 함께 로드한 뒤 `owner.profile_is_developer` / `owner.profile_explain_depth`를 socket `chat:turn` payload에 실어 보낸다. planning-agent `sio_handlers.on_chat_turn`이 수신해 프롬프트 최상단 블록(비개발자 금지 용어 / 개발자 허용 / brief 분량)으로 조립.
2. **프로젝트 제목 고정** — `project.title`을 프롬프트 상단에 선언, "일반화 금지" 명시(회고 §8.6: 랜덤 간식 당번을 "할 일 관리 앱"으로 부른 사례).
3. **`<EXISTING_PRD>` 블록 (UPDATE 모드만)** — 위 §3.3에서 언급한 PRD 선주입.

payload에 필드가 누락되면 planning-agent는 비개발자/detailed/null로 fallback — 구버전 orchestrator와의 호환성 유지.
...
- **컨테이너 명명 swap** (ADR 0008 §D4-ter, 2026-04-24 추가 — 회고 §10): update 사이클에서 새 컨테이너는 임시 이름(`project-{id}-update-{ts}`)으로 띄우고, 헬스체크 통과 직후 옛 컨테이너 제거 + 새 컨테이너를 canonical 이름(`project-{id}`)으로 swap-rename. Docker daemon의 컨테이너 이름 unique 제약과 §D4의 "두 컨테이너 동시 보유" 모델을 동시에 만족시키기 위함. `DockerService.canonicalName/createContainer/renameContainer` 3종 API로 표현.
...
| `planning_update / update_ready → deployed` | 유저 "↩ 업데이트 취소" (`POST /update/cancel`). session archived + 문서 복원 (ADR 0008 §D4-bis). |
...
**업데이트 사이클 유저 주도 취소 (ADR 0008 §D4-bis)**

유저가 업데이트 대화 도중 "방향이 틀렸다"고 판단해 이전 배포 상태로 되돌리고 싶을 때:

- 진입 시 자동 백업: `chat.service.archiveCurrentSessionForUpdate`가 `deployed → planning_update` 첫 진입 시점에 `projects/<id>/.ax-build/pre-update-backup/`에 PRD.md·DESIGN.md 스냅샷 저장. 이미 백업 있으면 skip (최초 원본 보존).
- 취소 엔드포인트 `POST /api/projects/:id/update/cancel`:
  1. state는 `planning_update` / `update_ready`만 허용
  2. 현재 session → `archived`
  3. 백업에서 PRD·DESIGN 복원 → 백업 디렉토리 제거
  4. `current_session_id` null + state → `deployed` 전이
  5. WS `progress(phase='update_cycle_cancelled')` emit → 다른 탭/협업자도 자동 대시보드 이동
- UI: Chat header 우측 "↩ 업데이트 취소" 버튼 (planning_update / update_ready + owner/editor). 확인 모달 승인 시 호출.
- 첫 빌드 라인엔 이 기능 없음 — draft/planning/plan_ready에서 방향이 틀렸으면 프로젝트 **삭제** 후 새로 시작하면 됨. 업데이트 라인만 "운영 중인 앱 유지한 채 이번 사이클만 폐기"가 의미 있음.
...
### 12.5 PRD 백업 API + 일관성 신호 (회고 §8, 2026-04-24)

ADR 0008 §D4-bis의 "업데이트 취소·문서 복원"이 `update` 사이클 범위의 롤백이라면, 이번 백업 레이어는 **모든 `write_prd` 호출마다 기록**되는 더 촘촘한 복원점이다. 두 레이어는 충돌하지 않는다 — D4-bis는 "사이클 시작 전으로"를 보장하고, 백업 레이어는 "직전 write로" 세분 복원.

- **엔드포인트 (orchestrator)**:
  - `GET /api/projects/:id/prd/backups` (owner/editor) — `[{filename, timestamp, bytes}]` 역순. filename 패턴 `PRD.md.bak.{YYYYMMDDTHHMMSSZ}` 외 거부.
  - `POST /api/projects/:id/prd/restore` (owner) — body `{filename}`. 복원 직전 현재 PRD도 `.bak`으로 보존해 복원의 복원 가능.
- **경로 조작 방어**: 컨트롤러에서 filename 정규식 검증 → service에서 `path.join(projectDir, filename)`만 허용.
- **`title_prd_mismatch` 시그널**: `projects.service.findOne` 응답에 boolean 필드로 포함. H1과 `project.title`의 정규화 비교 → H1이 generic 라벨(예: "제품 요구사항 정의 (PRD)")이면 본문 전체에서 title 언급 여부로 fallback. 프론트 Chat 페이지가 이 필드로 호박색 배너 + 복원 CTA 표시.
