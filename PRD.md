...
| `env_qa → planning_update` | classifier `code_bug` (수정 필요) |
| `env_qa → planning` | classifier `schema_bug` (PRD 원천 문제 — 극단) |
...
| `updating → planning_update` | classifier `code_bug` → 대화로 보강. 컨테이너 복구. |
| `update_qa → planning_update` | regression_fail → 어떤 엔드포인트가 깨졌는지 gap에 포함 |
| `updating → planning` | classifier `schema_bug` (극단). previous 복구. |
