...
CREATE TABLE projects (
  ...
  -- state (ADR 0008):
  --   첫 빌드 라인: 'draft' | 'planning' | 'plan_ready' | 'building' | 'qa'
  --   업데이트 라인: 'planning_update' | 'update_ready' | 'updating' | 'update_qa'
  --   env: 'awaiting_env' | 'env_qa'
  --   터미널: 'deployed' | 'failed'
  current_session_id TEXT DEFAULT NULL,
  current_version INTEGER DEFAULT 0,
  port INTEGER DEFAULT NULL,
  container_id TEXT DEFAULT NULL,
  -- ADR 0008: 업데이트 실패 시 롤백용. updating 진입 직전 백업,
  -- 성공 시 clear (project_versions 테이블에 이관).
  previous_container_id TEXT DEFAULT NULL,
...
GET    /api/projects/:id/prd/backups        → PRD.md 백업 스냅샷 목록 (owner/editor)
                                               · 회고 §8 후속 — write_prd 덮어쓰기 직전 자동 스냅샷
                                               · Response: { backups: [{ filename, timestamp, bytes }] }
                                               · filename 패턴: PRD.md.bak.{YYYYMMDDTHHMMSSZ}
POST   /api/projects/:id/prd/restore        → PRD.md를 지정 스냅샷으로 복원 (owner만)
                                               · Request: { filename }
                                               · 복원 직전 현재 PRD도 .bak으로 자동 보존 (복원의 복원 여지)
                                               · Response: { success, restored_from, pre_restore_backup }
```

`GET /api/projects/:id`의 응답에는 다음 보조 필드가 함께 실린다:
- `failure_reason: string[] | null` — state=failed일 때 마지막 실패 사유.
- `last_bounce: {build_id, finished_at, gap_list} | null` — 반송 배너용.
- `title_prd_mismatch: boolean` — `project.title`과 PRD.md의 도메인이 어긋난 상태. 회고 §8 사고(간식 당번 PRD가 Todo App으로 덮어써짐) 재발 가시화용. H1이 generic 라벨이면 본문에서 title 언급 여부로 fallback 판정.
...
POST   /api/projects/:id/update/cancel → 업데이트 사이클 취소 (ADR 0008 §D4-bis)
                                          planning_update/update_ready 상태에서만 허용
                                          • 현재 session archived
                                          • .ax-build/pre-update-backup/의 PRD·DESIGN 복원
                                          • state → deployed
                                          • WS progress(phase=update_cycle_cancelled) emit
                                          owner/editor 권한.
```
...
```
POST /api/projects/:id/build           → 빌드 시작
                                          첫 빌드: plan_ready → building
                                          업데이트: update_ready → updating  (ADR 0008)
POST /api/projects/:id/build/cancel    → 진행 중 빌드 중단
                                          building/qa → failed
                                          updating/update_qa → deployed(previous 복구) 또는 failed
POST /api/projects/:id/build/retry     → 실패한 빌드 재시도 (기획 수정 없이 같은 handoff 재사용)
                                          failed + 유효 handoff → plan_ready → building
                                          첫 빌드 라인만 지원. 업데이트 실패는 planning_update
                                          반송 경로로 해결.
GET  /api/projects/:id/build/status    → 현재 빌드 상태 + phase 목록
                                          응답 필드:
                                            active:               boolean (프로세스 Map에 등록돼 있는가)
                                            state:                ProjectState
                                            consecutive_failures: number (최근 성공 이후 failed/bounced/
                                                                   cancelled 개수 — ARCHITECTURE §7.6
                                                                   retry-first UX 플립 기준. 2 이상이면
                                                                   프론트가 "기획 대화로"를 primary로)
                                            build:                { id, status, started_at, finished_at,
                                                                   bounce_reason_gap_list? }
                                            phases:               [{ idx, name, status, started_at,
                                                                   finished_at }]
...
Planning 이벤트:
  ...
  propose_handoff 결과 이벤트 (DESIGN §1 원칙 #6 — UI 진실원):
  - progress(phase='plan_ready'):
      { detail, handoff_id, min_completeness, is_sufficient, accepted: true }
  - progress(phase='update_ready'):   (ADR 0008 업데이트 라인)
      { detail, handoff_id, min_completeness, is_sufficient, accepted: true }
  - progress(phase='handoff_rejected'):
      { detail, min_completeness?, is_sufficient?, has_unresolved?, accepted: false }
      // tool_result를 orchestrator가 파싱해 직접 emit. AI 텍스트 무관하게
      // UI 배너가 이 이벤트로 확정적 상태 표시 (회고 §6 환각 대응).

  환각·사이클 제어 이벤트 (2026-04-22 추가):
  - progress(phase='hallucination_detected'):
      { detail }
      // AI가 "<tool_name> 도구를 호출합니다" 텍스트 + 실제 tool_call 0회일 때
      // orchestrator가 감지해 emit. 유저에게 경고 배너 + 재시도 유도.
  - progress(phase='update_cycle_cancelled'):
      { detail }
      // POST /update/cancel 실행 후 emit. 다른 탭/협업자도 이 이벤트로
      // 자동 대시보드 이동.
...
Building 이벤트:
  ...
  - phase_end:        { phase_name: string, status: 'success' | 'failed',
                        ...
                        primary_endpoints?: string[] }  // ADR 0008 QA regression용
  ...
  - progress(phase='build_failed_retryable'):   // ARCHITECTURE §7.6 retry-first
      { detail, classifier, matched_rule, gap_list, mode, retryable: true }
      // code_bug/unknown/infra/transient — 첫 빌드 라인은 failed 상태에서
      // 유저 선택 대기. 2회 연속 실패 시 프론트가 CTA 플립.
  - progress(phase='bounce_back'):             // 업데이트 라인 또는 legacy
      { detail, classifier, matched_rule, gap_list, mode }
  - error(env_qa):    { kind: 'env_qa_failure',
                        ...
                        next_state: 'awaiting_env' | 'planning' | 'planning_update' | 'env_qa' | 'deployed',
                        mode: 'fresh' | 'update' | 'maintenance',
                        message: string }  // §8.3 ADR 0002 + ADR 0008
```
...
| 도구 | 설명 | 허용 |
|---|---|---|
| `write_prd` | PRD.md 생성/갱신 — UPDATE 모드에서는 도구-레벨 가드(similarity/H1) 통과해야 하며 매 호출마다 `.bak.{iso8601Z}` 자동 백업 | ✅ |
...
도구 핸들러는 `(ToolCtx, args)` 시그니처를 받는다. `ToolCtx`는 `project_id / session_id / is_update_mode` 3필드로, 핸들러가 현재 라인(첫 빌드 vs 업데이트)을 알고 분기할 수 있게 한다.

**`write_prd` 가드 (회고 §8 후속, 2026-04-24)**:
- UPDATE 모드에서 `difflib.SequenceMatcher` ratio `< 0.3` → reject(사실상 전체 재작성).
- UPDATE 모드에서 H1이 바뀌고 ratio `< 0.7` → reject(도메인 전환으로 판정).
- 거부 시 AI에게 구체 가이드 반환(변경 없는 섹션 원문 복제 or "새 프로젝트" 제안).
- 매 호출마다 기존 PRD가 있으면 `PRD.md.bak.{iso8601Z}`로 자동 백업. 유저는 §5.2 `GET /prd/backups` + `POST /prd/restore`로 1클릭 복원(owner).
...
### 6.8 런타임 프롬프트 조립 (2026-04-23~24, 회고 §8)

시스템 프롬프트는 고정 상수가 아니라 **턴마다 3개 축으로 동적 조립**된다. 조립 주체는 `planning-agent/app/agent/system_prompt.build_system_prompt()`.

1. **유저 페르소나 preamble** — orchestrator `chat.service`가 `owner.profile_is_developer` / `owner.profile_explain_depth` 값을 socket `chat:turn` payload에 실어 보내고, planning-agent가 프롬프트 최상단에 고정 블록으로 박는다.
   - 비개발자: 금지 용어 목록(단위 테스트·API·엔드포인트·스키마·컴포넌트·런타임 등) + "테스트" → "버튼 눌러서 동작 확인"으로 번역하는 규칙 + 되묻기는 기술 분류 대신 사용 장면으로.
   - 개발자: 기술 용어 허용, 의사결정 부담은 여전히 최소화.
   - `explain_depth=brief`면 "2~3문장으로 짧게" 지시 추가.
2. **프로젝트 제목 고정** — `project.title`을 프롬프트 상단에 선언하고 "일반화 금지(예: '할 일 관리 앱'으로 호칭 금지)"를 명시. PRD 바디만 보고 LLM이 임의로 일반화하는 환각 차단.
3. **`<EXISTING_PRD>` 블록 (UPDATE 모드만)** — `_load_existing_prd(project_id)`가 프로젝트 디렉토리의 PRD.md를 읽어 system prompt 말미에 XML-like 블록으로 주입. AI가 "못 읽었다"고 주장할 수 없는 구조. 원래 `UPDATE_SYSTEM_PROMPT`가 "`Read` 도구로 먼저 읽어라"고 지시했는데 Read 도구 자체가 registry에 없었던 실패(회고 §8.7)에 대한 구조적 대체.

이 조립 결과는 chat:turn마다 다시 계산되므로 유저 프로필 변경이나 PRD 외부 편집이 즉시 반영된다.
...
### 10.1 상태 머신 (ADR 0005 + 0006 + 0008)

두 개 라인이 있다. **첫 빌드 라인**(draft → deployed)과 **업데이트 라인**(deployed → deployed v+1). env 관련 상태는 양쪽 공용 사이드 트랜지션.
...
draft → planning → plan_ready → building → qa → deployed ←───────────┐
          ↑                          │                                 │
          └── bounce-back ─────┘                                       │
              (phase 실행 실패·code_bug·schema_bug)                     │
                                                                        │
[env 사이드] deployed ↔ env_qa ←→ deployed  (ADR 0006, mock-first)     │
             env_qa code_bug → planning_update (업데이트 라인 진입)     │
                                                                        │
[업데이트 라인]                                                         │
deployed → planning_update → update_ready → updating → update_qa ─────┘
                  ▲                              ▼          ▼
                  │              rollback (previous 복구, deployed 유지)
                  └── bounce (code_bug · schema_bug · regression_fail) ──┘

[레거시]  awaiting_env → env_qa → ...  (ADR 0005 이전 플로우 호환)
```

**핵심 불변식**
- **mock-first** (ADR 0005): 빌드는 env 없이도 mock으로 deployed까지 직행.
- **update 실패 시 기존 배포 유지** (ADR 0008): `updating` / `update_qa` 실패는 `deployed` 유지 + previous 복구. Planning 반송은 극단 케이스(schema_bug)만.
- **`modifying` state는 `planning_update`로 대체** (ADR 0008).

| 상태 | 라인 | 의미 |
|---|---|---|
| `draft` | 첫 빌드 | 프로젝트 생성 직후 |
| `planning` | 첫 빌드 | Planning Agent와 신규 기획 대화 |
| `plan_ready` | 첫 빌드 | 완성도 충분, "빌드 시작" 대기 |
| `building` | 첫 빌드 | Building Agent 실행 중 |
| `qa` | 첫 빌드 | 관찰 기반 QA (mock 상태 구조 검증) |
| `awaiting_env` | env (레거시) | ADR 0005 이전 블로킹 입력 대기 |
| `env_qa` | env | 저장+재시작 또는 awaiting_env 제출 후 헬스체크 |
| `deployed` | 터미널·재진입점 | 컨테이너 기동 완료. 첫 빌드와 update 라인 공통 종착/출발점 |
| `planning_update` | 업데이트 | 배포된 앱의 수정사항을 Planning Agent와 논의 (기존 PRD diff) |
| `update_ready` | 업데이트 | 수정 사양 확정, "업데이트 시작" 대기 |
| `updating` | 업데이트 | Building Agent가 변경 범위 적용 중 (기존 코드 보존) |
...
| `building → qa` | Building Agent가 QA phase 진입 시 (UI "검증 중" 배지 표시용) |
| `qa → env_qa` | 관찰 QA 통과, 컨테이너 기동 단계로 |
| `env_qa → deployed` | 헬스체크 통과 |
| `building / qa → failed` | `code_bug` / `unknown` / `infra_error` / `transient` 실패 (retry-first) |
| `building / qa → failed` | 유저 수동 중단(`POST /build/cancel`) |
| `failed → plan_ready` | `POST /build/retry` — 유효 handoff가 남아있으면 기획 수정 없이 재빌드 |
| `{building, qa, updating, update_qa} → failed` | orchestrator 재시작 시 고아 빌드 자동 정리 (startup 훅) |

> **code_bug retry-first 정책**: 이전에는 `code_bug` → `planning` 자동 반송이었으나, Claude Code 실행이 확률적이라 같은 PRD로 재빌드하면 풀리는 경우가 많음. 현재는 `failed`로 두고 유저가 "다시 빌드" vs "기획 대화로"를 **명시적 선택**. 2회 연속 실패 시 UI가 "기획 대화로"를 primary CTA로 뒤집음. (업데이트 라인은 여전히 `planning_update` 반송 — 대화 수정 흐름이 자연스러움.)

**env 사이드 (ADR 0005·0006)**

| 전환 | 트리거 |
|---|---|
| `deployed → deployed` | env 저장(재시작 없음) — DB만 |
| `deployed → env_qa` | "저장 후 재시작" 또는 `POST /restart` |
| `env_qa → deployed` | 헬스체크 통과 / transient · env_rejected 폴백 |
| `env_qa → planning_update` | classifier `code_bug` (update/maintenance 모드 — 수정 필요) |
| `env_qa → planning` | classifier `schema_bug` (fresh 모드 한정, PRD 원천 문제 — 극단) |
| `awaiting_env → env_qa` | (레거시) 초기 설정 모드 제출 |

**업데이트 라인 (ADR 0008)**

| 전환 | 트리거 |
|---|---|
| `deployed → planning_update` | 유저가 배포된 프로젝트 채팅에 첫 메시지 or "수정 요청" 클릭 |
| `planning_update → update_ready` | `propose_handoff(from=planning_update)` |
| `update_ready → updating` | "업데이트 시작" 클릭. previous_container_id/version 백업. |
| `updating → update_qa` | 변경 범위 적용 완료 |
| `update_qa → deployed` | regression 통과 + 새 기능 동작. 버전 +1. previous 필드 clear. |
| `update_qa → deployed` | classifier `infra_error` / `transient` → **기존 previous로 롤백** + 토스트. 앱은 계속 동작. |
| `updating → planning_update` | classifier `code_bug` / `schema_bug` / `unknown` → 대화로 보강. 컨테이너·버전 previous 복구. |
| `update_qa → planning_update` | regression_fail → 어떤 엔드포인트가 깨졌는지 gap에 포함. previous 복구. |
| `updating → deployed` | classifier `infra_error` / `transient` → previous 유지 + 토스트. 빌드 실패 없던 것처럼. |
| `planning_update → deployed` | 유저가 "↩ 업데이트 취소" 선택 (`POST /update/cancel`). session archived + .ax-build/pre-update-backup/ 문서 복원. ADR 0008 §D4-bis. |
| `update_ready → deployed` | 위와 동일 — update_ready 상태에서도 취소 가능 |
...
### Phase 6.1: 내부 에이전트 통합 + 운영성 + **업데이트 상태 라인 분리** (예정)

**6.1-A. 업데이트 라인 분리 (ADR 0008) — 우선순위 최상**
33. `ProjectState` union 확장: `planning_update` / `update_ready` / `updating` / `update_qa` 추가, `modifying` 제거(리네임)
34. `state-machine.service.ts` VALID_TRANSITIONS 재작성 (업데이트 라인 전이 + 롤백 경로)
35. `projects` 엔티티에 `previous_container_id` / `previous_version` 컬럼 (update 롤백 백업용)
36. `propose_handoff.py`: `planning_update → update_ready` 전이 추가 (현재 임시 `planning OR modifying` 분기 교체)
37. `chat.service.sendUserMessage`: `deployed` 첫 메시지 시 `planning_update` 전이 (임시 `modifying` 전이 교체)
38. `building.runner.start()`: mode = `build | update` 파라미터. update 모드는 `updating → update_qa` 경로
39. Planning Agent system_prompt — update 세션 전용 분기 섹션 (기존 PRD diff 원칙 명시)
40. Building Agent Hermes 프롬프트 — update 모드에서 기존 코드 보존 + 변경 범위 최소화
41. `env-deploy.service` — update 실패 시 previous로 롤백 (컨테이너 재생성 + 이전 버전 디렉토리 복원)
42. QA 확장 — `project_versions.primary_endpoints` JSON 컬럼 + update_qa에서 regression curl
43. 프론트 state config 확장 — 배지·카피·버튼 라벨 update 라인 전용
44. E2E 스모크에 "배포 → 수정 → 재배포" 사이클 추가 (회고 §3.2)

**6.1-B. Gateway·운영성**
45. **Planning / Hermes / Claude Code CLI의 Gateway 경유 전환**
...
46. **사용량·비용 로깅** — 호출마다 `ai_usage_logs` 행 기록 (project_id, model, tokens, cost_est, timestamp)
47. **프로젝트 단위 예산 cap** — 일일/월 한도 초과 시 429 + 저렴 모델 자동 폴백(opt-in)
48. **관리자 대시보드 카드** (DESIGN.md §5.2) — 이번 달 요청수·토큰·예상 비용
49. **Anthropic / OpenAI passthrough 엔드포인트** — `/api/ai/v1/anthropic/messages` 등, tool use/thinking/caching 원본 포맷 중계
50. **`agent-model-mcp` slot-routing backend 통합** — Gateway의 `model: "fast|deep|code|..."` 논리 이름을 MCP stdio의 slot dispatch로 매핑 (로컬 Mac Studio 도입 시 모델 전환 투명화)
51. **FailureClassifier LLM judge 2차** (`qa_judge` 슬롯) — regex 미매칭 로그를 LLM이 분류
52. **기능 단위 QA** — `npm install + health`를 넘어 생성 앱의 주요 엔드포인트 curl 검증 (PRD에 정의한 핵심 유저 플로우별)
