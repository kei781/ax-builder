...
QA는 앱이 어느 포트에 바인드할지 **주입하지 않고 관찰**한다. `npm start`로 뜬 프로세스의 pid 기준으로 실제 LISTEN 포트를 추출하고, 그 포트에 HTTP 응답을 확인한다. 앱이 env 관례를 지키지 않아도 QA가 통과할 수 있도록 하기 위함.
...
`.env.example`은 QA 선제 체크(§8.1)에서 존재 여부가 강제된다. 즉 빌드 산출물엔 반드시 있다. 아래 플로우는 **유저가 입력해야 하는(user-required) 변수가 있을 때만** 실행된다. AI Gateway 토큰처럼 시스템이 자동 주입하는 변수는 유저에게 보이지 않는다(§9.1.1).
...
```
관찰 기반 QA 통과 (§8.1)
  → .env.example 파싱 → 필수(required) 변수 감지
    → required 변수 0개: 곧바로 deployed
    → required 변수 1개 이상: status = 'awaiting_env'
      → 프론트엔드 ENV 입력 UI (§9.3 가이드 표시)
      → 유저 입력 → 암호화 저장 → .env 파일 생성 → 컨테이너 재시작
      → status = 'env_qa' → 재QA (§8.1과 동일 관찰 기반)
        → 통과: deployed
        → 실패: FailureClassifier(§8.3)로 분류
            ├── env_rejected → status='awaiting_env'로 복귀, 해당 변수 강조 표시 (최대 3회)
            ├── transient    → status='env_qa' 유지, 지수 백오프 재시도 + "재시도" 버튼
            ├── code_bug     → status='planning' (bounce-back), 스택 gap 첨부
            └── schema_bug   → status='planning' (bounce-back), 변수 이력 gap 첨부
```

**원칙**: env 단계 실패를 **기본적으로 기획 반송하지 않는다.** 유저가 고칠 수 있는 문제(env_rejected, transient)는 유저 루프 내에서 해결. AI가 고쳐야 하는 문제(code_bug, schema_bug)만 Planning으로 올라감.
...
draft → planning → plan_ready → building → qa ──┬─> awaiting_env → env_qa ─┬─> deployed
          ...
          │                          │        └─> awaiting_env ←────────────────────────── ┘
          │                          │
          └────── bounce-back (code_bug / schema_bug) ──────┘
                                                              │
                                                              └─> modifying (새 세션)
```

실패 분기는 모두 §8.3 FailureClassifier가 판정한다. transient는 상태 유지 + 재시도.
...
| `qa` | 관찰 기반 QA 진행 중 (§8.1) |
| `awaiting_env` | QA 통과, `.env.example`에 required 변수 존재 → 유저 입력 대기 |
| `env_qa` | 유저가 env 입력 후 재QA 중 |
| `deployed` | 컨테이너 기동 완료, 접속 가능 |
| `failed` | 해결 불가 상태 (분류기가 판단 불가 + 반복 실패 시) |
...
| `qa → awaiting_env` | 관찰 QA 통과 + `.env.example`에 required 변수 존재 |
| `qa → deployed` | 관찰 QA 통과 + required 변수 0개 |
| `awaiting_env → env_qa` | 유저가 env 값 입력 + 컨테이너 재시작 |
| `env_qa → deployed` | 재QA 통과 |
| `env_qa → awaiting_env` | FailureClassifier가 `env_rejected`로 판정 (누적 3회 미만) |
| `env_qa → env_qa` | FailureClassifier가 `transient`로 판정 (자동 재시도) |
| `qa / env_qa → planning` | FailureClassifier가 `code_bug` / `schema_bug`로 판정 → bounce-back |
